(() => {
  ...
  var init_utils = __esm({
    "node_modules/@trystero-p2p/core/dist/utils.mjs"() {
      ...
      genId = (n) => alloc(n, () => charSet[floor(Math.random() * 62)] ?? "").join("");
      selfId = genId(20);
      all = Promise.all.bind(Promise);
      isBrowser = typeof window !== "undefined";
      ({ entries, fromEntries, keys, values } = Object);
      noOp = () => {
      };
      ...
      mkErr = (msg) => /* @__PURE__ */ new Error(`${libName}: ${msg}`);
      toErrorMessage = (reason, fallback) => {
        if (reason instanceof Error && reason.message) return reason.message;
        if (typeof reason === "string" && reason) return reason;
        return toJson(reason ?? fallback);
      };
      toError = (reason, fallback) => reason instanceof Error ? reason : mkErr(toErrorMessage(reason, fallback));
      encoder = new TextEncoder();
      decoder = new TextDecoder();
      encodeBytes = (txt) => encoder.encode(txt);
      decodeBytes = (buffer) => decoder.decode(buffer);
      toHex = (buffer) => buffer.reduce((a, c) => a + c.toString(16).padStart(2, "0"), "");
      topicPath = (...parts) => parts.join("@");
      shuffle = (xs, seed) => {
        const a = [...xs];
        const rand = () => {
      ...
      getRelays = (config, defaults, defaultN, deriveFromAppId = false) => config.relayConfig?.urls || (deriveFromAppId ? shuffle(defaults, strToNum(config.appId)) : defaults).slice(0, config.relayConfig?.redundancy ?? defaultN);
      ...
      makeSocket = (url, onMessage, onReconnect) => {
        const client = {};
        let didOpen = false;
  ...
  var init_crypto = __esm({
    "node_modules/@trystero-p2p/core/dist/crypto.mjs"() {
      ...
      sha1 = async (str) => strToSha1[str] ??= Array.from(await hashWith("SHA-1", str)).map((b) => b.toString(36)).join("");
  ...
  var init_handshake = __esm({
    "node_modules/@trystero-p2p/core/dist/handshake.mjs"() {
      ...
      createPasswordHandshake = (password, appId, roomId) => {
      ...
      toHandshakeErrorMessage = (error) => {
        const message = toErrorMessage(error, "unknown error");
        return message.startsWith("handshake ") ? message : `handshake failed: ${message}`;
      };
      createHandshakeManager = ({ onPeerHandshake, onHandshakeError, handshakeTimeoutMs, sendHandshakeData, sendHandshakeReady, onActivate, onFailure }) => {
        const peerStates = {};
  ...
  var init_peer = __esm({
    "node_modules/@trystero-p2p/core/dist/peer.mjs"() {
      ...
      iceStateEvent = "icegatheringstatechange";
      iceConnectionStateEvent = "iceconnectionstatechange";
      offerType = "offer";
      answerType = "answer";
      outOfRangePattern = /out of range/i;
      rewriteMdnsCandidatesToLoopback = (sdp) => sdp.replace(/ (\S+\.local) (\d+) typ host/g, " 127.0.0.1 $2 typ host");
      peer_default = (initiator, { trickleIce, rtcConfig, rtcPolyfill, turnConfig, _test_only_mdnsHostFallbackToLoopback }) => {
      ...
      defaultIceServers = [...alloc(3, (_, i) => `stun:stun${i || ""}.l.google.com:19302`), "stun:stun.cloudflare.com:3478"].map((url) => ({ urls: url }));
    }
  });
  ...
  var init_action_wire = __esm({
    "node_modules/@trystero-p2p/core/dist/action-wire.mjs"() {
      ...
      channelCloseEvent = "close";
      channelErrorEvent = "error";
      backpressureWaitTimeoutMs = 1e4;
      toByteArray = (value) => value instanceof ArrayBuffer ? new Uint8Array(value) : new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
      waitForBufferedAmountLow = (channel, timeoutMs = backpressureWaitTimeoutMs) => {
      ...
      createActionWireManager = ({ getPeer, getPeerIds, canReceiveFromPeer, throwIfAborted: throwIfAborted2 }) => {
        const actions = {};
        const actionsCache = {};
  ...
  var init_actions = __esm({
    "node_modules/@trystero-p2p/core/dist/actions.mjs"() {
      ...
      withMetadata = (context, metadata) => metadata === void 0 ? context : {
        ...context,
        metadata
      };
      createActionManager = ({ getPeer, getPeerIds, canReceiveFromPeer }) => {
        const publicActions = {};
  ...
  var init_media = __esm({
    "node_modules/@trystero-p2p/core/dist/media.mjs"() {
      ...
      createMediaManager = ({ iterate, isActive, getSharedMediaPeer }) => {
  ...
  var init_room = __esm({
    "node_modules/@trystero-p2p/core/dist/room.mjs"() {
      ...
      defaultHandshakeTimeoutMs = 1e4;
      internalNs = (ns) => "@_" + ns;
      beforeUnloadRoomCleanups = /* @__PURE__ */ new Set();
      cleanupActiveRoomsOnBeforeUnload = () => beforeUnloadRoomCleanups.forEach((cleanup) => cleanup());
      ...
      room_default = (onPeer, onPeerLeave, onSelfLeave, { onPeerHandshake, onHandshakeError, handshakeTimeoutMs = defaultHandshakeTimeoutMs, isPassive = false } = {}) => {
        const peerMap = {};
        const activePeerMap = {};
        const pendingPongs = {};
        const listeners = {
  ...
  var init_shared_peer = __esm({
    "node_modules/@trystero-p2p/core/dist/shared-peer.mjs"() {
      ...
      init_media();
      roomFrameVersion = 1;
      roomPresenceFrameVersion = 2;
      wrapRoomFrame = (roomToken, data) => {
      ...
      wrapRoomPresenceFrame = (roomToken, isPresent) => {
        const tokenBytes = encodeBytes(roomToken);
  ...
  var init_signal_handler = __esm({
    "node_modules/@trystero-p2p/core/dist/signal-handler.mjs"() {
      ...
      init_offer_pool();
      init_shared_peer();
      offerPostAnswerTtlMs = 23333;
      offerIdSize = 12;
      disconnectedPeerGraceMs = 7533;
      answeringTtlMs = 23333;
      legacyCandidateKey = "__legacy__";
      offerRelayPlaceholder = "offer-placeholder";
      signalKeys = [
        "offer",
        "answer",
        "candidate"
      ...
      getString = (payload, key) => typeof payload[key] === "string" && payload[key] ? payload[key] : void 0;
      hasInvalidSignalField = (payload) => signalKeys.some((key) => key in payload && (typeof payload[key] !== "string" || payload[key] === ""));
      publishCipheredSignalingMessage = (ctx, signal, peerTopic, signalPeer, buildPayload, stillValid) => {
      ...
      getSdpExchangeConnectionError = (peerId, config) => `could not connect to peer ${peerId} after exchanging SDP; ${hasTurnServer(config) ? "check that your TURN server URLs and credentials are reachable by both peers" : "configure TURN servers with turnConfig or rtcConfig.iceServers"}`;
      ...
      getState = (peerStates, peerId) => peerStates[peerId] ??= makeState();
      updateStatus = (state) => {
      ...
      clearConnectedPeer = (state, peerId, _reason) => {
      ...
      resetOfferState = (state, offerPool) => {
      ...
      scheduleAnsweringExpiry = (ctx, state, peerId, peer) => {
      ...
      flushBufferedCandidates = async (state, peer, offerId) => {
      ...
      scheduleOfferExpiry = (ctx, state, peerId, ttlMs = offerTtl) => {
      ...
      handleAnnouncement = async (ctx, relayId, peerId, shared, signalPeer) => {
        if (shared) {
      ...
      handleOffer = async (ctx, relayId, peerId, offer, offerId, hasOutgoingOfferHint, signalPeer) => {
      ...
      handleCandidate = async (ctx, peerId, candidate, offerId, peer) => {
        let plainCandidate;
        try {
      ...
      handleAnswer = async (ctx, relayId, peerId, answer, offerId, peer) => {
        let plainAnswer;
        try {
      ...
      createSignalHandler = (ctx) => (relayId) => async (topic, msg, signalPeer) => {
        if (ctx.isLeaving()) return;
  ...
  var init_strategy = __esm({
    "node_modules/@trystero-p2p/core/dist/strategy.mjs"() {
      ...
      announceWarmupIntervalsMs = [
        233,
        533,
        1333
      ];
      passiveActivationGraceMs = 7533;
      sharedPeerIdleMsDefault = 123333;
      strategy_default = ({ init, subscribe, announce, deactivate }) => {
        const occupiedRooms = {};
  ...
  var relayManager, topicToInfoHash, infoHashToTopic, announceFns, subscriptionTokens, trackerAnnounceMs, handledSignals, msgHandlers, topicStates, roomOutstandingOffers, roomOfferGenerationPromises, roomSubscriberCounts, trackerAction, hashLimit, offerPoolSize, defaultAnnounceMs, dormantAnnounceMs, offerRetentionMs, signalDedupeWindowMs, defaultRedundancy, getInfoHash, send, warn, getRoomOutstandingOffers, deleteRoomOfferBookkeeping, claimOutstandingOffer, reclaimOutstandingOffer, reclaimAllOutstandingOffers, pruneOutstandingOffers, ensureOutstandingOffers, joinRoomStrategy, joinRoom, getRelaySockets, defaultRelayUrls;
  var init_dist2 = __esm({
    "node_modules/@trystero-p2p/torrent/dist/index.mjs"() {
      ...
      topicToInfoHash = {};
      infoHashToTopic = {};
      announceFns = relayManager.scoped();
      subscriptionTokens = relayManager.scoped();
      trackerAnnounceMs = relayManager.scoped();
      handledSignals = {};
      msgHandlers = relayManager.scoped();
      topicStates = relayManager.scoped();
      roomOutstandingOffers = {};
      roomOfferGenerationPromises = {};
      roomSubscriberCounts = {};
      trackerAction = "announce";
      hashLimit = 20;
      offerPoolSize = 3;
      defaultAnnounceMs = 1e4;
      dormantAnnounceMs = 12e4;
      offerRetentionMs = 12e4;
      signalDedupeWindowMs = 4e3;
      defaultRedundancy = 3;
      getInfoHash = async (topic) => {
      ...
      send = async (client, topic, payload) => client.send(toJson({
        action: trackerAction,
        info_hash: await getInfoHash(topic),
        peer_id: selfId,
        ...payload
      }));
      warn = (url, msg, didFail = false) => console.warn(`${libName}: torrent tracker ${didFail ? "failure" : "warning"} from ${url} - ${msg}`);
      getRoomOutstandingOffers = (rootTopic) => roomOutstandingOffers[rootTopic] ??= {};
      deleteRoomOfferBookkeeping = (rootTopic) => {
        delete roomOutstandingOffers[rootTopic];
        delete roomOfferGenerationPromises[rootTopic];
      };
      claimOutstandingOffer = (rootTopic, offerId) => {
      ...
      ensureOutstandingOffers = async (rootTopic, getOffers) => {
      ...
      joinRoomStrategy = strategy_default({
      ...
      joinRoom = (config, roomId, callbacks) => joinRoomStrategy({
        ...config,
  ...
  var require_renderer = __commonJS({
    "renderer.js"() {
      init_dist2();
      var config = { appId: "sync-yndmusic-dev-test" };
      var room = joinRoom(config, "test-room-1");
      var [sendHello, getHello] = room.makeAction("hello");
      var statusEl = document.getElementById("status");
      room.onPeerJoin((peerId) => {
        console.log("\u041F\u0438\u0440 \u043F\u043E\u0434\u043A\u043B\u044E\u0447\u0438\u043B\u0441\u044F:", peerId);
        statusEl.textContent = "\u041F\u0438\u0440 \u043F\u043E\u0434\u043A\u043B\u044E\u0447\u0438\u043B\u0441\u044F: " + peerId;
        sendHello("\u041F\u0440\u0438\u0432\u0435\u0442 \u043E\u0442 " + Math.floor(Math.random() * 1e3));
      });
      room.onPeerLeave((peerId) => {
        console.log("\u041F\u0438\u0440 \u043E\u0442\u043A\u043B\u044E\u0447\u0438\u043B\u0441\u044F:", peerId);
      });
      getHello((data, peerId) => {
        console.log("\u041F\u043E\u043B\u0443\u0447\u0435\u043D\u043E \u0441\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u043E\u0442", peerId, ":", data);
        statusEl.textContent = '\u041F\u043E\u043B\u0443\u0447\u0435\u043D\u043E \u0441\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435: "' + data + '" \u043E\u0442 ' + peerId;
      });
    }
  });
  require_renderer();
